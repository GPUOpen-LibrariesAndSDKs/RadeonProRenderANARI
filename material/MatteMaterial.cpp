...
rpr_material_node MatteMaterial::generateMaterial(Geometry *geometry){
  ...
  {
    CHECK(rprMaterialNodeSetInputNByKey(material, RPR_MATERIAL_INPUT_UBER_DIFFUSE_COLOR, m_diffuse_color_sampler->generateMaterial(geometry)))
  }
  else if(!m_diffuse_color_attribute.empty() && geometry->hasAttribute(m_diffuse_color_attribute.c_str()))  // color is a geometry attribute
  {
  ...
  // opacity
  if((!m_opacity_attribute.empty() && geometry->hasAttribute(m_opacity_attribute.c_str())) || m_opacity_sampler)  // opacity is a material node
  {
    rpr_material_node opacityNode = m_opacity_sampler ? m_opacity_sampler->generateMaterial(geometry) : geometry->getAttribute(m_opacity_attribute.c_str())->getMaterial();
